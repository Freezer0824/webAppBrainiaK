import { useMemo, useState } from "react";
import { Cpu, Power, RefreshCw, RotateCcw, Send, Trash2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useRuntimeStore } from "@/store/runtime-store";
import {
  activateMode,
  deactivateMode,
  getModes,
  resetModes,
  setModes,
} from "@/lib/api/modes-api";

const DEFAULT_MODES_PAYLOAD = {
  modes: ["reasoning"],
};

function prettyJson(value: unknown) {
  return JSON.stringify(value, null, 2);
}

function previewPayload(value: unknown) {
  try {
    return prettyJson(value).slice(0, 600);
  } catch {
    return String(value).slice(0, 600);
  }
}

function parseJsonInput(value: string) {
  try {
    return {
      ok: true as const,
      value: JSON.parse(value) as unknown,
    };
  } catch (error) {
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : "JSON invalide",
    };
  }
}

function extractModes(payload: unknown): string[] {
  if (!payload || typeof payload !== "object") return [];

  const data = payload as Record<string, unknown>;
  const candidates = [data.modes, data.active_modes, data.activeModes, data.data];

  const array = candidates.find(Array.isArray);

  if (!Array.isArray(array)) return [];

  return array.filter((item): item is string => typeof item === "string");
}

export function ModesDashboard() {
  const [modeName, setModeName] = useState("reasoning");
  const [modesPayload, setModesPayload] = useState(
    prettyJson(DEFAULT_MODES_PAYLOAD),
  );
  const [result, setResult] = useState<unknown>(null);
  const [activeModes, setActiveModes] = useState<string[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [loadingAction, setLoadingAction] = useState<string | null>(null);

  const pushApiEvent = useRuntimeStore((state) => state.pushApiEvent);

  const parsedModesPayload = useMemo(
    () => parseJsonInput(modesPayload),
    [modesPayload],
  );

  const [lastAction, setLastAction] = useState<null | (() => Promise<void>)>(null);

  async function runAction(
    action: string,
    route: string,
    method: string,
    callback: () => Promise<unknown>,
  ) {
    const startedAt = performance.now();

    setLastAction(() => async () => {
      await runAction(action, route, method, callback);
    });    

    setLoadingAction(action);
    setError(null);

    pushApiEvent({
      module: "Modes",
      action,
      method,
      route,
      status: "running",
      httpStatus: null,
      durationMs: null,
      payloadPreview: null,
      error: null,
    });

    try {
      const payload = await callback();
      const durationMs = Math.round(performance.now() - startedAt);

      setResult(payload);

      const extracted = extractModes(payload);
      if (extracted.length > 0) {
        setActiveModes(extracted);
      }

      pushApiEvent({
        module: "Modes",
        action,
        method,
        route,
        status: "success",
        httpStatus: 200,
        durationMs,
        payloadPreview: previewPayload(payload),
        error: null,
      });

      return payload;
    } catch (actionError) {
      const durationMs = Math.round(performance.now() - startedAt);
      const message =
        actionError instanceof Error ? actionError.message : "Erreur inconnue.";

      setError(message);

      pushApiEvent({
        module: "Modes",
        action,
        method,
        route,
        status: "error",
        httpStatus: null,
        durationMs,
        payloadPreview: null,
        error: message,
      });

      return null;
    } finally {
      setLoadingAction(null);
    }
  }

  async function handleRefreshModes() {
    await runAction("refresh-modes", "GET /v1/modes", "GET", getModes);
  }

  async function handleActivateMode() {
    const name = modeName.trim();

    if (!name) {
      setError("Aucun nom de mode à activer.");
      return;
    }

    await runAction(
      "activate-mode",
      "POST /v1/modes/{mode_name}",
      "POST",
      () => activateMode(name),
    );
    await handleRefreshModes();
  }

  async function handleDeactivateMode(nameFromList?: string) {
    const name = (nameFromList ?? modeName).trim();

    if (!name) {
      setError("Aucun nom de mode à désactiver.");
      return;
    }

    const confirmed = window.confirm(
      `Désactiver le mode "${name}" côté backend ?`,
    );

    if (!confirmed) return;

    await runAction(
      "deactivate-mode",
      "DELETE /v1/modes/{mode_name}",
      "DELETE",
      () => deactivateMode(name),
    );

    await handleRefreshModes();
  }

  async function handleSetModes() {
    if (!parsedModesPayload.ok) {
      setError(`JSON invalide : ${parsedModesPayload.error}`);
      return;
    }

    await runAction(
      "set-modes",
      "POST /v1/modes",
      "POST",
      () => setModes(parsedModesPayload.value),
    );
    await handleRefreshModes();
  }

  async function handleResetModes() {
    const confirmed = window.confirm(
      "Réinitialiser tous les modes actifs côté backend ?",
    );

    if (!confirmed) return;

    await runAction("reset-modes", "DELETE /v1/modes", "DELETE", resetModes);
    setActiveModes([]);
    await handleRefreshModes();
  }

  function handleResetInput() {
    setModesPayload(prettyJson(DEFAULT_MODES_PAYLOAD));
    setModeName("reasoning");
    setError(null);
    setLastAction(null);
  }

  return (
    <section className="scrollbar-brainiak min-h-0 flex-1 overflow-y-auto bg-[var(--surface-0)] p-6">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="rounded-3xl border border-[var(--border)] bg-[var(--surface-2)] p-6 shadow-xl">
          <p className="mb-2 text-xs font-semibold uppercase tracking-[0.24em] text-cyan-300">
            Brainiak Control Console
          </p>

          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
            <div>
              <h1 className="heading-brainiak text-3xl text-[var(--text-primary)]">
                Modes
              </h1>

              <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">
                Contrôler les modes actifs de Brainiak, activer un mode précis,
                remplacer la liste complète ou réinitialiser l’état.
              </p>
            </div>

            <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-1)] px-4 py-3 text-sm">
              <div className="text-xs uppercase tracking-[0.16em] text-[var(--text-secondary)]">
                État connexion
              </div>
              <div className="mt-1 font-medium text-[var(--text-primary)]">
                {loadingAction ? "Action en cours..." : "Prêt"}
              </div>
            </div>
          </div>

          <div className="mt-6 flex flex-wrap gap-2">
            {[
              "GET /v1/modes",
              "POST /v1/modes",
              "POST /v1/modes/{mode_name}",
              "DELETE /v1/modes/{mode_name}",
              "DELETE /v1/modes",
            ].map((route) => (
              <span
                key={route}
                className="rounded-full border border-cyan-500/30 bg-cyan-500/10 px-3 py-1 text-xs font-medium text-cyan-200"
              >
                {route}
              </span>
            ))}
          </div>
        </header>

        <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
          <aside className="space-y-6">
            <div className="rounded-3xl border border-[var(--border)] bg-[var(--surface-2)] p-5">
              <div className="flex items-center justify-between gap-3">
                <h2 className="text-sm font-semibold text-[var(--text-primary)]">
                  Modes actifs
                </h2>

                <Button
                  type="button"
                  size="sm"
                  variant="outline"
                  onClick={() => void handleRefreshModes()}
                  disabled={loadingAction !== null}
                  className="border-[var(--border)] bg-transparent"
                >
                  <RefreshCw className="mr-2 h-4 w-4" />
                  Refresh
                </Button>
              </div>

              <div className="mt-4 space-y-2">
                {activeModes.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface-1)] p-4 text-sm text-[var(--text-secondary)]">
                    Aucun mode actif chargé.
                  </div>
                ) : (
                  activeModes.map((mode) => (
                    <div
                      key={mode}
                      className="flex items-center justify-between gap-3 rounded-2xl border border-[var(--border)] bg-[var(--surface-1)] px-4 py-3"
                    >
                      <div className="flex min-w-0 items-center gap-2">
                        <Cpu className="h-4 w-4 shrink-0 text-cyan-300" />
                        <span className="truncate font-mono text-sm text-[var(--text-primary)]">
                          {mode}
                        </span>
                      </div>

                      <button
                        type="button"
                        onClick={() => void handleDeactivateMode(mode)}
                        disabled={loadingAction !== null}
                        className="rounded-lg px-2 py-1 text-xs text-red-300 hover:bg-red-500/10 disabled:opacity-50"
                      >
                        Désactiver
                      </button>
                    </div>
                  ))
                )}
              </div>
            </div>

            <div className="rounded-3xl border border-[var(--border)] bg-[var(--surface-2)] p-5">
              <h2 className="text-sm font-semibold text-[var(--text-primary)]">
                Mode individuel
              </h2>

              <label className="mt-4 block">
                <span className="text-xs text-[var(--text-secondary)]">
                  mode_name
                </span>

                <input
                  value={modeName}
                  onChange={(event) => setModeName(event.target.value)}
                  placeholder="reasoning"
                  className="mt-2 w-full rounded-2xl border border-[var(--border)] bg-[var(--surface-1)] px-4 py-3 text-sm text-[var(--text-primary)] outline-none placeholder:text-[var(--text-secondary)] focus:border-cyan-500/50"
                />
              </label>

              <div className="mt-4 grid gap-2">
                <Button
                  type="button"
                  onClick={() => void handleActivateMode()}
                  disabled={loadingAction !== null || !modeName.trim()}
                  className="bg-gradient-brainiak text-black hover:opacity-90 disabled:opacity-50"
                >
                  <Power className="mr-2 h-4 w-4" />
                  Activate Mode
                </Button>

                <Button
                  type="button"
                  variant="outline"
                  onClick={() => void handleDeactivateMode()}
                  disabled={loadingAction !== null || !modeName.trim()}
                  className="border-[var(--border)] bg-transparent"
                >
                  Deactivate Mode
                </Button>
              </div>
            </div>
          </aside>

          <div className="space-y-6">
            <div className="rounded-3xl border border-[var(--border)] bg-[var(--surface-2)] p-5">
              <div className="mb-3 flex items-center justify-between gap-3">
                <h2 className="text-sm font-semibold text-[var(--text-primary)]">
                  Set Modes JSON
                </h2>

                <Button
                  type="button"
                  size="sm"
                  variant="outline"
                  onClick={handleResetInput}
                  className="border-[var(--border)] bg-transparent"
                >
                  <RotateCcw className="mr-2 h-4 w-4" />
                  Reset Input
                </Button>
              </div>

              <textarea
                value={modesPayload}
                onChange={(event) => setModesPayload(event.target.value)}
                spellCheck={false}
                className="min-h-[220px] w-full resize-y rounded-2xl border border-[var(--border)] bg-[var(--surface-1)] p-4 font-mono text-sm leading-6 text-[var(--text-primary)] outline-none focus:border-cyan-500/50"
              />

              {!parsedModesPayload.ok ? (
                <p className="mt-3 text-sm text-red-300">
                  JSON invalide : {parsedModesPayload.error}
                </p>
              ) : null}

              <div className="mt-4 flex flex-wrap gap-2">
                <Button
                  type="button"
                  onClick={() => void handleSetModes()}
                  disabled={loadingAction !== null || !parsedModesPayload.ok}
                  className="bg-gradient-brainiak text-black hover:opacity-90 disabled:opacity-50"
                >
                  <Send className="mr-2 h-4 w-4" />
                  Set Modes
                </Button>

                <Button
                  type="button"
                  variant="outline"
                  onClick={() => void handleResetModes()}
                  disabled={loadingAction !== null}
                  className="border-red-500/30 bg-transparent text-red-300 hover:bg-red-500/10"
                >
                  <Trash2 className="mr-2 h-4 w-4" />
                  Reset All Modes
                </Button>
              </div>
            </div>

            <div className="rounded-3xl border border-[var(--border)] bg-[var(--surface-2)] p-5">
              <h2 className="text-sm font-semibold text-[var(--text-primary)]">
                Résultat brut
              </h2>

              <pre className="mt-3 max-h-[420px] overflow-auto rounded-2xl border border-[var(--border)] bg-[var(--surface-1)] p-4 text-xs leading-6 text-[var(--text-secondary)]">
                {result ? prettyJson(result) : "Aucun résultat pour le moment."}
              </pre>
            </div>
          </div>
        </div>

        {error ? (
          <div className="rounded-3xl border border-red-500/20 bg-red-500/10 p-5">
            <h2 className="text-sm font-semibold text-red-200">
              Message d’erreur
            </h2>

            <p className="mt-2 text-sm leading-6 text-red-100/80">{error}</p>

            {lastAction ? (
              <Button
                type="button"
                variant="outline"
                onClick={() => void lastAction()}
                disabled={loadingAction !== null}
                className="mt-4 border-red-500/30 bg-transparent text-red-200 hover:bg-red-500/10"
              >
                Réessayer
              </Button>
            ) : null}
          </div>
        ) : null}
      </div>
    </section>
  );
}